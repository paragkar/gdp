...
if plot_type == "Scatter":
   ...
   # Generate scatter plots with trendlines for each dimension
    ...
    if timescale == "Quarter":
        ...
        # Assuming the years are integers and converting them to datetime objects at the end of March
    ...
    for i, dimension in enumerate(pivot_df.index, 1):
        ...
        # Generate timestamps for linear regression if necessary
        timestamps = [x.timestamp() for x in x_data] if timescale == "FYear" else x_data
        ...
        trend = np.polyfit(timestamps, y_data, 1) if timescale == "FYear" else np.polyfit(x_data, y_data, 1)
        trendline = np.poly1d(trend)(timestamps) if timescale == "FYear" else np.poly1d(trend)(x_data)
        fig.add_trace(go.Scatter(x=x_data, y=trendline, mode='lines', name=f'{dimension} Trend'), row=i, col=1)

    # Define and add rectangles for each plot
