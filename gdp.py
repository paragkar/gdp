...
def plotingscatter(pivot_df, dimension,timescale,currency,feature):
    ...
    fig.update_layout(
        ...
        title_text= title_text,
        showlegend=False,
        margin=dict(t=40, l=50, r=50, b=50)) # Reduce the top margin (t) value as needed
...
def plotingscatterforecast(pivot_df, dimension, timescale, currency, feature, forecast_period):
    ...
    if timescale == "Quarter":
        ...
        st.write("Selected Quarters in Range: ", len(selected_cols),", Start Date: ",selected_cols[0].date(), ", End Date : ", selected_cols[-1].date())
